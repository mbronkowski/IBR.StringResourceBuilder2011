...
namespace IBR.StringResourceBuilder2011.Modules
{
    class Parser
    {
        ...
        private const int SQL_RES_MAX_LENGTH = 256;
        ...
        private void ParseSqlScriptForStrings(TextPoint startPoint,
                                     TextPoint endPoint,
                                     int lastDocumentLength)
        {
            //0.35-0.06 seconds (threaded: 2.47-1.77 seconds)
            ...
            if (isTextWithStringLiteral)
            {
                TextDocument doc = (TextDocument)(m_Window.Document.Object("TextDocument"));
                var ePoint = startPoint.CreateEditPoint();
                string sqlContent = ePoint.GetText(endPoint);

                ProcessSqlErrorsMessages(sqlContent, m_DoProgress, stringResources, m_Settings, startLine, endLine);
        ...
        private void ProcessSqlErrorsMessages(string script,
        ...
        int endLine)
        {
            ...
            _EncodeDocument(stringResources, lines);
            //stringResources.Add();
        }

        private void _EncodeDocument(List<StringResource> stringResources, List<string> documentLines)
        {
            for (int lineIndex = 0; lineIndex < documentLines.Count; ++lineIndex)
            {
                string line = documentLines[lineIndex];
                List<SqlPhrase> phrases = _FindPhrases(line, lineIndex);
                ...
                {
                    SqlPhrase phrase = phrases[phraseIndex];

                    // ewentualny tekst przed pierwszą frazą
                    if (phraseIndex == 0 && phrase.Start > 0)
                        newLineSB.Append(line.Substring(0, phrase.Start));

                    string encodedText = _EncodeRawText(stringResources, phrase.Text, lineIndex, phrase.Start, line);
                    if (encodedText == null)
                    {
                        // nie udało się zakodować - pozostaw niezakodowany
                        encodedText = phrase.Text;
                    }

                    newLineSB.Append(encodedText);

                    int postPhraseStart = phrase.Start + phrase.Text.Length;
                    if (phraseIndex < phrases.Count - 1)
                    {
                        // nie ostatnia fraza
                        int postPhraseLenght = phrases[phraseIndex + 1].Start - postPhraseStart;
                        newLineSB.Append(line.Substring(postPhraseStart, postPhraseLenght));     // do początku następnej frazy
                    }
                    else
                    {
                        // ostatnia fraza
                        newLineSB.Append(line.Substring(postPhraseStart));       // do końca linii
                    }
                }
                documentLines[lineIndex] = newLineSB.ToString();
            }
        }

        private string _EncodeRawText(List<StringResource> stringResources, string rawText, int lineIdx, int rowIdx, string lineText)
        {
            string encArgs = "";
            string resKey = "";
            string resValue = "";
            _ParseRawText(rawText, ref encArgs, ref resKey, ref resValue);

            // czy w zasobach jest taka wartość?
            var resource = _FindFirstResourceByValue(m_existingSqlResources, stringResources, resValue);
            var resLocation = new System.Drawing.Point(lineIdx + 1, rowIdx);
            if (resource != null)
            {
                // TAK, jest wartość - weź jako bieżący klucz z zasobu i aktualizuj zasób
                resKey = resource.Item1;
                stringResources.Add(new StringResource(resKey, resValue, resLocation, lineText, false, string.Format("'${0}{1}'", resKey, encArgs), rawText));
            }
            else
            {
                // NIE, nie ma wartości - poszukaj po kluczu

                // czy jest klucz?
                resource = _FindResourceByKey(m_existingSqlResources, stringResources, resKey);
                if (resource != null)
                {
                    // TAK, jest klucz - sprawdź czy nie ma konfliku na wartościach

                    // czy wartości się różnią?
                    if (resource.Item2 != resValue)
                    {
                        // TAK, wartości różnią się - mamy konflikt wartości

                        // spróbuj wymyśleć nowy klucz, a jeżeli się nie uda zaprotestuj
                        string uniqResKey = _GetUniqResKey(m_existingSqlResources, stringResources, resKey);
                        if (uniqResKey == null)
                        {
                            return null;
                        }

                        // udało się wymyśleć nowy klucz - dodaj zasób z nowym kluczem i weź nowy klucz jao bieżący
                        resKey = uniqResKey;
                        stringResources.Add(new StringResource(resKey, resValue, resLocation, lineText, false, string.Format("'${0}{1}'", resKey, encArgs), rawText));
                    }
                }
                ...
                {
                    // NIE, w zasobach brak klucza - dodaj zasób
                    stringResources.Add(new StringResource(resKey, resValue, resLocation, lineText, false, string.Format("'${0}{1}'", resKey, encArgs), rawText));
                }
            }

            return string.Format("${0}{1}", resKey, encArgs);
        }

        public static Tuple<string, string> _FindFirstResourceByValue(Dictionary<string, string> existingResources, List<StringResource> resources, string value)
        {
            foreach (var resource in resources)
                if (resource.Text == value)
                    return new Tuple<string, string>(resource.Name, resource.Text);

            foreach (var resource in existingResources)
                if (resource.Value == value)
                    return new Tuple<string, string>(resource.Key, resource.Value);

            return null;
        }

        public static Tuple<string, string> _FindResourceByKey(Dictionary<string, string> existingResources, List<StringResource> resources, string key)
        {
            foreach (StringResource resource in resources)
                if (resource.Name.ToUpper() == key.ToUpper())
                    return new Tuple<string, string>(resource.Name, resource.Text);

            foreach (var resource in existingResources)
                if (resource.Key.ToUpper() == key.ToUpper())
                    return new Tuple<string, string>(resource.Key, resource.Value);

            return null;
        }

        private static string _GetUniqResKey(Dictionary<string, string> existingResources, List<StringResource> resources, string baseResKey)
        {
            for (int i = 1; i < 100; ++i)
            {
                string key = string.Format("{0}{1}", baseResKey, i);
                if (_FindResourceByKey(existingResources, resources, key) == null)
                    return key;
            }
            return null;
        }

        private List<SqlPhrase> _FindPhrases(string documentLine, int lineNo)
        {
            List<SqlPhrase> phrases = new List<SqlPhrase>();

            string trimedLine = documentLine.TrimStart();
            if (trimedLine.Trim() == "" || trimedLine.StartsWith("--") || trimedLine.StartsWith("/*") || !trimedLine.ToLower().StartsWith("raiserror"))
                return null;

            StringBuilder phraseSB = new StringBuilder();
            bool inPhrase = false;
            bool inBlockComment = false;
            int phraseFirstApostropheIndex = 0;
            for (int i = 0; i < documentLine.Length; ++i)
            {
                char c1 = documentLine[i];
                char c2 = i < documentLine.Length - 1 ? documentLine[i + 1] : '\n';
                ...
                if (inPhrase)
                {
                    if (c1 == '\'')
                    {
                        ...
                        {
                            phrases.Add(new SqlPhrase(phraseFirstApostropheIndex + 1, phraseSB.ToString(), lineNo));
                            phraseSB.Clear();
                            inPhrase = false;
                        }
                    }
                ...
                else if (c1 == '\'')
                {
                    ...
                    {
                        phraseFirstApostropheIndex = i;
                        inPhrase = true;
                    }
                }
            }
            return phrases.Where(x => !x.Text.StartsWith("$")).ToList();
        }

        private void _ParseRawText(string rawText, ref string encArgs, ref string resKey, ref string resValue)
        {
            string key = "";
            string last = "";
            string mesg = "";
            ...
            for (int i = 0; i < rawText.Length; ++i)
            {
                ...
                else if (last == "%" && rawText[i] == '%')
                {
                    mesg += '%';
                    last = "";
                    key += "_";
                }
                else if (last == "%" && "diosuxX".Contains(rawText[i]))
                {
                    mesg += string.Format("{0}{1}{2}", '{', nArgs, '}');
                    args += string.Format("|%{0}", rawText[i]);
                    last = "";
                    key += string.Format("{0}", nArgs);
                    ++nArgs;
                }
                else
                {
                    mesg += rawText[i];
                    last = "";
                    key += char.IsLetterOrDigit(rawText[i]) ? rawText[i] : '_';
                }
            }
            encArgs = args;
            if (key.Length > SQL_RES_MAX_LENGTH)
                resKey = key.Substring(0, SQL_RES_MAX_LENGTH).ToUpper();
            else
                resKey = key.ToUpper();
            resValue = mesg;
        }
          ...
          if (isIgnoreMethodArguments || string.IsNullOrEmpty(part.Trim()))
            ...
            if (txt.Length > 0)
            {
                ...
                for (int i = 0; i < parts.Length; ++i)
                {
                    ...
                    stringResources.Add(new StringResource(name, draftName, new System.Drawing.Point(lineNo, stringPos[i] + colNo - 1), txtLine, IsAttribut, null, draftName));
                } //for
                #endregion
            } //if
        }
                                                      ...
                                                      ref int parentheses)
    {
      ...
      return (txtPart);
        ...
        public static void ParseSqlScriptForStrings(Window window,
                                        List<StringResource> stringResources,
                                        Dictionary<string, string> existingResources,
                                        Settings settings,
                                        Action<int> progressCallback,
                                        Action<bool> completedCallback,
                                        TextPoint startPoint,
                                        ...
                                        int lastDocumentLength)
        {
            Parser parser = new Parser(window, stringResources, settings, progressCallback, completedCallback, existingResources);
            parser.ParseSqlScriptForStrings(startPoint, endPoint, lastDocumentLength);
        }
