...
namespace IBR.StringResourceBuilder2011.Modules
{
    class StringResourceBuilder
    {
      ...
      if (!m_Window.Caption.EndsWith(".xaml"))
        ...
        private void BrowseSqlScriptForStrings(EnvDTE.Window window,
                                            TextPoint startPoint,
                                            TextPoint endPoint)
        {
            Debug.Print("\n> BrowseSqlScriptForStrings()");

            if (m_IsBrowsing)
            {
                Debug.Print("> BrowseSqlScriptForStrings() - cancelled (already running)");
                return;
            } //if
            ...
            var resxProjItemPath = OpenOrCreateSqlResourceFile()?.Document?.FullName;
            var existingResources = GetSqlResources(resxProjItemPath);
            Parser.ParseSqlScriptForStrings(m_Window, m_StringResources, existingResources, m_Settings, this.SetProgress, ParsingCompleted,
                                    startPoint, endPoint, m_LastDocumentLength);

            //store for next operation (moving locations)
            m_LastDocumentLength = m_TextDocument.EndPoint.Line;

            Debug.Print("> BrowseSqlScriptForStrings() - ended\n");
        }
                ...
                using (System.IO.StreamReader file = new System.IO.StreamReader(Settings.AiWorkFolder + "result.csv"))
                {
                    ...
                    while ((line = file.ReadLine()) != null)
                    {
                        ...
                        if (Settings.AiMinimumPrecision <= precision)
                        {
                            ...
                            {
                                mStringResources[counter].AISugestionAsAt = true;
                                if(!mStringResources[counter].ForSql)
        ...
        private static bool IsSqlLanguage(Document document)
        {
            if (document == null)
                return false;

            if (document.Language == "SQL Server Tools")
                return true;
        ...
        private bool IsSqlFile(EnvDTE.Window window)
        {
            Debug.Print(">>>> IsSqlFile() - window='{0}' active='{1}'", (window == null) ? "null" : window.Caption, (m_Dte2.ActiveDocument == null) ? "null" : m_Dte2.ActiveDocument.ActiveWindow.Caption);
            ...
            if (m_Window == null)
            {
                if (!IsSqlLanguage(m_Dte2.ActiveDocument))
                    return (false);
            ...
            Debug.Print("m_TextDocument.Selection.ActivePoint.Line = {0}",
        ...
        public ProjectItem OpenOrCreateSqlResourceFile()
        {
            string solutionDir = Path.GetDirectoryName(m_Dte2.Solution.FullName);

            ProjectItem resourceFilePrjItem = null;

            var settingPath = string.Format(@"{0}\{1}", solutionDir, m_Settings.SqlResPath);

            resourceFilePrjItem = m_Dte2.Solution.FindProjectItem(settingPath);
        ...
        private void BuildAndUseSqlResource(ProjectItem resourceFilePrjItem, StringResource resource = null)
        {
            try
            {
                string resxFile = resourceFilePrjItem.Document.FullName;
                ...
                string name,
                       value,
                       comment = string.Empty;
                StringResource stringResource = resource ?? m_SelectedStringResource;
                if (stringResource.SkipAsAt)
                {
                    m_TextDocument.Selection.MoveToLineAndOffset(stringResource.Location.X, stringResource.Location.Y, false);
                    UpdateTableAndSelectNext(1, 0);
                ...
                if (replaceLength > 0)
                {
                    var resourceCall = stringResource.SqlCodedValue;
                    int oldRow = m_TextDocument.Selection.ActivePoint.Line;
                    //insert the resource call, replacing the selected string literal
                    m_TextDocument.Selection.Insert(resourceCall, (int)vsInsertFlags.vsInsertFlagsContainNewText);
        ...
        private Dictionary<string, string> GetSqlResources(string resourceFileName)
        {
            ...
            try
            {
                ...
                foreach (DictionaryEntry d_loopVariable in rsxr)
                {
                    result.Add(d_loopVariable.Key.ToString(), d_loopVariable.Value.ToString());
                }
                return result;
            }
            catch (Exception)
        ...
        public void DoBrowse()
        {
            ...
            else if (IsSqlFile(m_FocusedTextDocumentWindow))
            {
                BrowseSqlScriptForStrings(m_FocusedTextDocumentWindow, m_TextDocument.StartPoint, m_TextDocument.EndPoint);
            }
            else
            {
                ClearStringResources();
        ...
        public bool IsSqlFile()
        {
            return IsSqlLanguage(m_Dte2.ActiveDocument);
        }
        ...
        public void SelectStringInTextDocument(StringResource resource = null)
        {
            ...
            string text = stringResource.ForSql ? stringResource.RawText : stringResource.Text;
        ...
        public void BuildAndUseResource()
        {
            ...
            if (IsSqlLanguage(m_Window.Document))
            {
                ...
                BuildAndUseSqlResource(resourceFilePrjItem);
            }
            else
            {
